...
function emailHtml(input: EmailInput) {
  const action = input.action
    ? `<tr><td style="padding:8px 32px 32px"><a href="${escapeHtml(
        input.action.url,
      )}" style="display:inline-block;background:#2B241C;color:#F6EEDF;text-decoration:none;padding:13px 20px;border-radius:8px;font-weight:600">${escapeHtml(
        input.action.label,
      )}</a></td></tr>`
    : "";
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(
    input.subject,
  )}</title></head><body style="margin:0;background:#F6EEDF;color:#2B241C;font-family:Arial,sans-serif"><div style="display:none;max-height:0;overflow:hidden">${escapeHtml(
    input.preheader,
  )}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#F6EEDF;padding:24px 12px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fffaf0;border:1px solid #DFCFAE;border-radius:12px;overflow:hidden"><tr><td style="padding:28px 32px;border-bottom:1px solid #DFCFAE"><div style="font-family:Georgia,serif;font-size:26px;color:#7A2530">Divine <span style="color:#B8862E">Karigari</span></div><div style="margin-top:5px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#706657">Thoughtful gifts, beautifully made</div></td></tr><tr><td style="padding:32px 32px 12px"><h1 style="margin:0;font-family:Georgia,serif;font-size:32px;line-height:1.2;font-weight:500">${escapeHtml(
    input.heading,
